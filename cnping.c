...
int load_ping_packet( uint8_t *buffer, int bufflen )
{
	...
	{
		PingSendTimes[ ( current_cycle + PINGCYCLEWIDTH - 1 ) & ( PINGCYCLEWIDTH - 1 ) ] =
			0; // Unset ping send.
	}
...
void DrawFrameHistogram()
{
	...
	else
	{
		...
		for ( i = slotsmin; i <= slotsmax; i++ )
		{
			...
			uint64_t total = 0;
			ssmsMIN[ tslot ] = samp;
			for ( j = 0; j < skips; j++ ) { total += hist_counts[ samp++ ]; }
		...
		for ( i = 0; i < rslots; i++ )
		{
			...
			if ( !in_frame_mode ) { CNFGColor( 0xffffffff ); }
			else { CNFGColor( 0x8080ffff ); }


			CNFGPenX = startx + ( 8 - log10 ) * 4;
			CNFGPenY = bottom + 3;
#ifdef WIN32
		...
		if ( !in_frame_mode ) DrawMainText( stt );
		return;
	}
nodata:
...
void DrawFrame( void )
{
	...
	for ( i = 0; i < screenx; i++ )
	{
		...
		if ( !GuiYscaleFactorIsConstant ) { GuiYScaleFactor = ( screeny - 50 ) / windmaxtime; }
...
#endif
		"Loss:%6.1f %%",
		last, pinghost, mintime, maxtime, globmaxtime * 1000, avg, globinterval * 1000.0, stddev,
...
{
	HKEY hKey;
	if ( RegCreateKeyExA( HKEY_CURRENT_USER, "Software\\cnping", 0, NULL, REG_OPTION_NON_VOLATILE,
...
INT_PTR CALLBACK TextEntry( HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	switch ( uMsg )
	{
		case WM_INITDIALOG:
			...
			char data[ 1024 ];
			if ( !RegString( 0, data, sizeof( data ) ) ) { SetDlgItemText( hwndDlg, 3, data ); }
		...
		case WM_COMMAND:
			...
			{
				case 4:
				case 3: return 0; // keyboard input
				case 1:
				case 2: return 0; // focus changed.
				case 0:
				{
					...
					char Address[ 128 ];
					GetDlgItemText( hwndDlg, 3, Address, sizeof( Address ) );
					char Period[ 128 ];
					GetDlgItemText( hwndDlg, 4, Period, sizeof( Period ) );
					char Extra[ 128 ];
					GetDlgItemText( hwndDlg, 5, Extra, sizeof( Extra ) );
					char Scaling[ 128 ];
					GetDlgItemText( hwndDlg, 6, Scaling, sizeof( Scaling ) );
...
int main( int argc, const char **argv )
{
...
#ifndef DEBUG
	for ( i = 0; i < sizeof( pattern ); i++ ) { pattern[ i ] = rand(); }
#endif
	...
	if ( title[ 0 ] == 0 ) { sprintf( title, "%s - cnping " VERSION, pinghost ); }

	if ( GuiYScaleFactor > 0 ) { GuiYscaleFactorIsConstant = 1; }

	if ( !pinghost ) { displayhelp = 1; }
	...
	while ( 1 )
	{
		...
		if ( in_frame_mode ) { DrawFrame(); }

		if ( in_histogram_mode ) { DrawFrameHistogram(); }
