...
void DoHTTPing( const char *addy, double minperiod, int *seqnoptr, volatile double *timeouttime,
	int *socketptr, volatile int *getting_host_by_name, const char *device )
{
#if defined( WIN32 ) || defined( WINDOWS )
	...
	*getting_host_by_name = 1;
	serverresolve =
		resolveName( (struct sockaddr *)&serveraddr, &serveraddr_len, hostname, AF_UNSPEC );
...
reconnect:
	*socketptr = httpsock = socket( serveraddr.sin6_family, SOCK_STREAM, 0 );
	...
	if ( device )
	{
		if ( setsockopt( httpsock, SOL_SOCKET, SO_BINDTODEVICE, device, strlen( device ) + 1 ) !=
			 0 )
		{
	...
	while ( 1 )
	{
		...
		*timeouttime = OGGetAbsoluteTime();
		...
		if ( delay_time > 0 ) usleep( (int)( delay_time * 1000000 ) );
		if ( !breakout )
		{
#ifdef WIN32
...
static void *DeployPing( void *v )
{
	...
	DoHTTPing( hpl->addy, hpl->minperiod, &hpl->seqno, &hpl->timeout_time, &hpl->socket,
		&hpl->getting_host_by_name, hpl->device );
	hpl->failed = 1;
	return 0;
}
...
static void *PingRunner( void *v )
{
	...
	while ( 1 )
	{
		...
		while ( 1 )
		{
			...
			if ( usl > 0 )
				usleep( (int)( usl * 1000000 + 1000 ) );
			else
				usleep( 10000 );

			if ( hpl->timeout_time + HTTPTIMEOUT < Now &&
				 !hpl->getting_host_by_name ) // Can't terminate in the middle of a gethostbyname
			                                  // operation otherwise bad things can happen.
			{
				if ( hpl->socket )
				{
#ifdef WIN32
