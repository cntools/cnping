...
void ping_setup( const char *strhost, const char *device )
{
	...
	resolveName(
		(struct sockaddr *)&psaddr, &psaddr_len, strhost, AF_INET ); // only resolve ipv4 on windows
...
static void *pingerthread( void *v )
{
	...
	while ( 1 )
	{
		...
		DWORD res = IcmpSendEcho( ih, ( (struct sockaddr_in *)&psaddr )->sin_addr.s_addr,
			ping_payload, rl, 0, &repl, sizeof( repl ), timeout_ms );
		int err;
		...
		if ( !res )
		{
			if ( err == 11050 ) { printf( "GENERAL FAILURE\n" ); }
			else { printf( "ERROR: %d\n", err ); }
		}
		if ( res ) { display( repl.rply.Data, rl ); }
		OGUnlockSema( s_disp );
	}
	return 0;
}
...
uint16_t checksum( const unsigned char *start, uint16_t len )
{
	...
	uint32_t csum = 0;
	for ( i = 1; i < len; i += 2 ) csum += (uint32_t)( *( wptr++ ) );
...
void setTTL( int sock )
{
	...
	if ( setsockopt( sd, ( psaddr.sin6_family == AF_INET ) ? SOL_IP : SOL_IPV6, IP_TTL, &val,
			 sizeof( val ) ) != 0 )
	{
...
int createSocket()
{
	if ( psaddr.sin6_family == AF_INET ) { return socket( PF_INET, SOCK_RAW, IPPROTO_ICMP ); }
...
void listener()
{
	...
	keep_retry_quick:
		...
		else { ERRM( "Error: recvfrom failed" ); }
...
void ping( struct sockaddr *addr, socklen_t addr_len )
{
...
#else
	if ( fcntl( sd, F_SETFL, O_NONBLOCK ) != 0 ) ERRM( "Warning: Request nonblocking I/O failed." );
#endif
	...
	do {
		...
		memset( &pckt.hdr, 0, sizeof( pckt.hdr ) ); // This needs to be here, but I don't know why,
	...
	// close( sd ); //Hacky, we don't close here because SD doesn't come from here, rather  from
	// ping_setup.  We may want to run this multiple times.
}
